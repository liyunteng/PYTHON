...
# a.py - capture chrome input, gather info from ssh, then send to weixin-work
...
def send_markdown_weixin(message):
    tm = time.localtime()
...
def conver_msg_to_image(msg, filename):
    width = 1920
    heigth = 1080
    n = len(msg.split('\r\n'))
    heigth = (((n // 35) + 1) * 700)
    print("{}  {}".format(n, heigth))
    msg = msg.replace('\r\n', '\n')

    im = Image.new("RGB", (width, heigth), (0, 0, 0))
    ...
    dr.text((50, 50), msg, font=font, fill='#ffffff')
    im.save(filename)
...
if __name__ == '__main__':
    ...
    for x in msg:
        ...
        with open(os.path.join(basedir, x+'.txt'), 'w') as f:
            f.write(msg[x])
        conver_msg_to_image(msg[x], os.path.join(basedir, x+'.png'))
        send_image_weixin(os.path.join(basedir, x+'.png'))
